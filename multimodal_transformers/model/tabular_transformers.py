...
from transformers.models.audio_spectrogram_transformer.modeling_audio_spectrogram_transformer import AUDIO_SPECTROGRAM_TRANSFORMER_INPUTS_DOCSTRING
...
class ASTWithTabular(ASTForAudioClassification):
    """
    FIXME: Edit to fit AST.
    Roberta Model transformer with a sequence classification/regression head as well as
    a TabularFeatCombiner module to combine categorical and numerical features
    with the Roberta pooled output
    ...
    @add_start_docstrings_to_model_forward(
        AUDIO_SPECTROGRAM_TRANSFORMER_INPUTS_DOCSTRING.format("(batch_size, sequence_length)")
    )
    def forward(self,
                input_values=None,          # Model-specific
                head_mask=None,             # Model-specific
                labels=None,                # Model-specific
                output_attentions=None,     # Model-specific
                output_hidden_states=None,  # Model-specific
                class_weights=None,         # Tabular-specific
                cat_feats=None,             # Tabular-specific
                numerical_feats=None        # Tabular-specific
    ):
        r"""
            FIXME: Edit to fit AST.
            class_weights (:obj:`torch.FloatTensor` of shape :obj:`(tabular_config.num_labels,)`, `optional`, defaults to :obj:`None`):
        ...
        Returns:
            ...
            classifier_layer_outputs(:obj:`list` of :obj:`torch.FloatTensor`):
                ...
                combining module's output
        """
        outputs = self.audio_spectrogram_transformer(input_values=input_values,
                                                     head_mask=head_mask,
                                                     output_attentions=output_attentions,
                                                     output_hidden_states=output_hidden_states)
